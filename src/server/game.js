...
class Game {
  ...
  lookUpWord(word)
  {
    ...
    if (word.length > 1)
    {
      filePath = path.join('DictionaryData', word[0].toUpperCase(), word[1].toUpperCase() + '.txt');

    }
    else //this shouldn't ever happen since all words played must be longer than 1 letter
    {
      filePath = path.join('DictionaryData', word[0].toUpperCase(), word[0].toUpperCase() + '.txt');
    }
